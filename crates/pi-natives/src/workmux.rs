...
//!
//! # Overview
//! Provides N-API bindings for workmux library functionality, enabling
...
impl From<BackendType> for WorkmuxBackend {
	fn from(bt: BackendType) -> Self {
		match bt {
			BackendType::Tmux => WorkmuxBackend::Tmux,
			BackendType::WezTerm => WorkmuxBackend::WezTerm,
			BackendType::Kitty => WorkmuxBackend::Kitty,
		}
	}
}
...
impl From<WorkmuxBackend> for BackendType {
	fn from(wb: WorkmuxBackend) -> Self {
		match wb {
			WorkmuxBackend::Tmux => BackendType::Tmux,
			WorkmuxBackend::WezTerm => BackendType::WezTerm,
			WorkmuxBackend::Kitty => BackendType::Kitty,
		}
	}
}
...
impl From<WorkmuxAgentStatus> for AgentStatus {
	fn from(status: WorkmuxAgentStatus) -> Self {
		match status {
			WorkmuxAgentStatus::Working => AgentStatus::Working,
			WorkmuxAgentStatus::Waiting => AgentStatus::Waiting,
			WorkmuxAgentStatus::Done => AgentStatus::Done,
		}
	}
}
...
impl From<AgentStatus> for WorkmuxAgentStatus {
	fn from(status: AgentStatus) -> Self {
		match status {
			AgentStatus::Working => WorkmuxAgentStatus::Working,
			AgentStatus::Waiting => WorkmuxAgentStatus::Waiting,
			AgentStatus::Done => WorkmuxAgentStatus::Done,
		}
	}
}
...
///
/// # Returns
/// Environment info including backend type, running status, and current pane ID.
#[napi(js_name = "workmuxDetectEnvironment")]
pub fn workmux_detect_environment() -> task::Async<WorkmuxEnvironment> {
	task::blocking("workmux.detect_environment", (), move |_| {
...
#[napi(js_name = "workmuxCreateWindow")]
pub fn workmux_create_window(params: WorkmuxCreateWindowParams) -> task::Async<String> {
	task::blocking("workmux.create_window", (), move |_| {
		let backend_type = detect_backend();
		let mux = create_backend(backend_type);
...
#[napi(js_name = "workmuxIsAvailable")]
pub fn workmux_is_available() -> task::Async<bool> {
	task::blocking("workmux.is_available", (), move |_| {
		let backend_type = detect_backend();
		let mux = create_backend(backend_type);
...
#[napi(js_name = "workmuxCurrentPaneId")]
pub fn workmux_current_pane_id() -> task::Async<Option<String>> {
	task::blocking("workmux.current_pane_id", (), move |_| {
		let backend_type = detect_backend();
		let mux = create_backend(backend_type);
...
#[napi(js_name = "workmuxSetAgentStatus")]
pub fn workmux_set_agent_status(
	status: WorkmuxAgentStatus,
	title: Option<String>,
) -> task::Async<()> {
	task::blocking("workmux.set_agent_status", (), move |_| {
		let backend_type = detect_backend();
		let mux = create_backend(backend_type);
...
#[napi(js_name = "workmuxClearAgentStatus")]
pub fn workmux_clear_agent_status() -> task::Async<()> {
	task::blocking("workmux.clear_agent_status", (), move |_| {
		let backend_type = detect_backend();
		let mux = create_backend(backend_type);
...
#[napi(js_name = "workmuxListAgents")]
pub fn workmux_list_agents() -> task::Async<Vec<WorkmuxAgentInfo>> {
	task::blocking("workmux.list_agents", (), move |_| {
...
#[napi(js_name = "workmuxSendKeys")]
pub fn workmux_send_keys(pane_id: String, keys: String) -> task::Async<()> {
	task::blocking("workmux.send_keys", (), move |_| {
		let backend_type = detect_backend();
		let mux = create_backend(backend_type);
...
#[napi(js_name = "workmuxCapturePane")]
pub fn workmux_capture_pane(pane_id: String, lines: Option<u32>) -> task::Async<Option<String>> {
	task::blocking("workmux.capture_pane", (), move |_| {
		let backend_type = detect_backend();
		let mux = create_backend(backend_type);
...
#[napi(js_name = "workmuxWindowExists")]
pub fn workmux_window_exists(prefix: String, name: String) -> task::Async<bool> {
	task::blocking("workmux.window_exists", (), move |_| {
		let backend_type = detect_backend();
		let mux = create_backend(backend_type);
...
#[napi(js_name = "workmuxSelectWindow")]
pub fn workmux_select_window(prefix: String, name: String) -> task::Async<()> {
	task::blocking("workmux.select_window", (), move |_| {
		let backend_type = detect_backend();
		let mux = create_backend(backend_type);
...
#[napi(js_name = "workmuxKillWindow")]
pub fn workmux_kill_window(full_name: String) -> task::Async<()> {
	task::blocking("workmux.kill_window", (), move |_| {
		let backend_type = detect_backend();
		let mux = create_backend(backend_type);
